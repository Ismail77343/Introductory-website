@extends('layouts.admin', ['title' => $isEditing ? __('admin.edit_language') : __('admin.add_language')])

@section('content')
    <section class="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="rounded-[2rem] border border-white/10 bg-white/5 p-6 lg:p-8">
            <h1 class="text-4xl font-black text-white">{{ $isEditing ? __('admin.edit_language') : __('admin.new_language') }}</h1>
            ...
            <form method="POST" action="{{ $isEditing ? route('admin.languages.update', $language) : route('admin.languages.store') }}" class="mt-8 grid gap-5 md:grid-cols-2">
                ...
                <div><label class="mb-2 block text-sm text-slate-300">{{ __('admin.sort_order') }}</label><input type="number" name="sort_order" value="{{ old('sort_order', $language->sort_order) }}" class="w-full rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-white"></div>
                ...
                <label class="flex items-center gap-3 text-slate-300 md:col-span-2">
                    ...
                    {{ __('admin.default_site_language') }}
                </label>

                <div class="md:col-span-2 flex gap-3">
                    <button class="rounded-2xl bg-amber-400 px-8 py-4 font-bold text-slate-950">{{ $isEditing ? __('admin.save_changes') : __('admin.add_language') }}</button>
                    <a href="{{ route('admin.languages.index') }}" class="rounded-2xl border border-white/10 px-8 py-4 font-bold text-white">{{ __('admin.cancel') }}</a>
                </div>
            </form>
        </div>
    </section>
