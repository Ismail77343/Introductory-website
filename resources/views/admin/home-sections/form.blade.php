@extends('layouts.admin', ['title' => $isEditing ? 'تعديل قسم رئيسي' : 'إضافة قسم رئيسي'])
...
@section('content')
    ...
    <section class="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="rounded-[2rem] border border-white/10 bg-white/5 p-6 lg:p-8">
            <h1 class="text-4xl font-black text-white">{{ $isEditing ? 'تعديل قسم من الصفحة الرئيسية' : 'إضافة قسم جديد للرئيسية' }}</h1>
            <p class="mt-3 text-slate-300">يمكنك هنا إنشاء أقسام ديناميكية متعددة اللغات مع صور وروابط وملفات مرفقة ورفع مباشر من لوحة التحكم.</p>
            ...
            <form method="POST" action="{{ $isEditing ? route('admin.home-sections.update', $section) : route('admin.home-sections.store') }}" enctype="multipart/form-data" class="mt-8 space-y-8">
                ...
                <div class="space-y-6 rounded-[1.75rem] border border-white/10 bg-slate-950/40 p-5">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-400">يمكنك كتابة نسخة مستقلة لكل لغة، وسيعرض الموقع اللغة المختارة تلقائيًا.</p>
                    </div>
                ...
                <div class="space-y-5">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-400">لكل عنصر صورة ومرفق وروابط، مع ترجمات مستقلة لنصوص الكروت أو الإحصائيات.</p>
                        </div>
                        <button type="button" id="add-item-button" class="rounded-2xl bg-white/10 px-6 py-3 font-bold text-white">إضافة كرت جديد</button>
                    </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.home-sections.index') }}" class="rounded-2xl border border-white/10 px-8 py-4 font-bold text-white">إلغاء</a>
                </div>
            </form>
        </div>
    </section>
