@extends('layouts.admin', ['title' => __('admin.about_sections_title')])

@section('content')
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="mb-8 flex items-center justify-between gap-4">
            <div>
                <h1 class="text-4xl font-black text-white">{{ __('admin.about_sections_title') }}</h1>
                <p class="mt-2 text-slate-300">{{ __('admin.about_sections_intro') }}</p>
            </div>
            <a href="{{ route('admin.about-sections.create') }}" class="rounded-2xl bg-amber-400 px-6 py-3 font-bold text-slate-950">{{ __('admin.add_section') }}</a>
        </div>
                        ...
                        <tr class="border-t border-white/10 text-sm text-slate-200">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ route('admin.about-sections.edit', $section) }}" class="rounded-xl bg-white/10 px-3 py-2">{{ __('admin.edit') }}</a>
                                    <form method="POST" action="{{ route('admin.about-sections.destroy', $section) }}" onsubmit="return confirm('{{ __('admin.confirm_delete_section') }}')">
                                        @csrf
                                        @method('DELETE')
