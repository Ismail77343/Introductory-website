...
class AdminHomeSectionController extends Controller
{
    ...
    public function index()
    {
        return view('admin.home-sections.index', [
            'sections' => HomeSection::query()->withCount('items')->orderBy('sort_order')->get(),
        ]);
    }
    ...
    public function create()
    {
        return view('admin.home-sections.form', [
            'section' => new HomeSection(['variant' => 'cards', 'is_active' => true, 'page' => 'home']),
            'items' => [],
            'isEditing' => false,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.home-sections.index')->with('success', 'تمت إضافة القسم بنجاح.');
    }
    ...
    public function edit(HomeSection $homeSection)
    {
        return view('admin.home-sections.form', [
            'section' => $homeSection,
            'items' => $homeSection->items()->orderBy('sort_order')->get()->toArray(),
            'isEditing' => true,
        ]);
    }
    ...
    {
        $data = $this->validatedData($request, $homeSection);

        DB::transaction(function () use ($homeSection, $data) {
            $homeSection->update($data['section']);
            $homeSection->items()->delete();
            $homeSection->items()->createMany($data['items']);
        });
